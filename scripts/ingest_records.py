...
def _ensure_raw_snapshots_and_sections(
    session,
    candidates: tuple,
    source_id_resolver: callable,
) -> dict[str, str]:
    """Create raw_snapshots and source_sections entries for the candidates.

    Returns a mapping from the original source_section_id to the new UUID
    ...
    from unibot.db.models import RawSnapshot, SourceSection

    section_id_map: dict[str, str] = {}
    source_snapshots: dict[str, str] = {}

    for candidate in candidates:
        orig_id = candidate.source_section_id
        if not orig_id or orig_id in section_id_map:
            continue
        ...
        new_uuid = str(uuid4())
        section_id_map[orig_id] = new_uuid
        ...
        if source_id not in source_snapshots:
            snapshot = RawSnapshot(
                ...
                storage_uri=candidate.source_url,
                storage_backend="memory",
                page_content_hash="stub",
            )
            session.add(snapshot)
            session.flush()
            source_snapshots[source_id] = snapshot.snapshot_id

        section = SourceSection(
            source_section_id=new_uuid,
            snapshot_id=source_snapshots[source_id],
            source_id=source_id,
            section_label=candidate.source_section_label or orig_id or "body",
            section_type=candidate.record_type,
            source_locator=candidate.source_locator,
            section_order=0,
            text_content=str(candidate.record_payload.get("content", "")),
            source_text_hash=candidate.value_hash,
            parser_backend="ingest_records",
            grounding_data={},
        )
        session.add(section)
    session.flush()
...
def ingest_candidates(session, candidates: tuple) -> int:
    ...
    section_id_map = _ensure_raw_snapshots_and_sections(session, candidates, source_id_resolver)

    # Replace source_section_id strings with generated UUIDs
    remapped = []
    for c in candidates:
        orig = c.source_section_id
        if orig and orig in section_id_map:
            remapped.append(VerificationCandidate(
                record_id=c.record_id,
                record_version_id=c.record_version_id,
                record_type=c.record_type,
                conflict_scope_id=c.conflict_scope_id,
                dedupe_key=c.dedupe_key,
                value_hash=c.value_hash,
                source_authority_tier=c.source_authority_tier,
                source_url=c.source_url,
                source_locator=c.source_locator,
                source_section_id=section_id_map[orig],
                source_section_label=c.source_section_label,
                cycle_label=c.cycle_label,
                effective_from=c.effective_from,
                effective_to=c.effective_to,
                fetched_at=c.fetched_at,
                parent_record_id=c.parent_record_id,
                parent_source_url=c.parent_source_url,
                year_confidence=c.year_confidence,
                page_content_hash=c.page_content_hash,
                record_payload=c.record_payload,
            ))
        else:
            remapped.append(c)
    ...
    cu.process_scope_updates(
        ...
        create_verification_event=lambda s, decision: cu.create_verification_event(s, decision),
    )
